...
Drop-in replacement for model2.py with two targeted changes:
  ...
  2. All activations replaced by tanh (bounded in (-1, 1)), matching Ben's VAE2.
     Eliminates dead-ReLU neurons and prevents unbounded activation blowup.
...
class VariationalAutoEncoder2(nn.Module):
    ...
    def __init__(self,
        ...
        # ------------------------------------------------------------------ #
        self.conv1 = nn.Conv1d(1,  32,  kernel_size=9, stride=2, padding=4,
                               padding_mode='reflect', bias=False)
        self.norm1 = nn.InstanceNorm1d(32, affine=True)

        self.conv2 = nn.Conv1d(32, 64,  kernel_size=9, stride=2, padding=4,
                               padding_mode='reflect', bias=False)
        self.norm2 = nn.InstanceNorm1d(64, affine=True)

        self.conv3 = nn.Conv1d(64, 128, kernel_size=9, stride=2, padding=4,
                               padding_mode='reflect', bias=False)
        self.norm3 = nn.InstanceNorm1d(128, affine=True)

        # Encoder: fully connected (no bias) + tanh
        self.dense1 = nn.Linear(128 * (input_dim // 8), hidden_dim1, bias=False)
        ...
        # Output heads (bias=True, zero-initialised for poles)
    ...
    def encode(self, x):
        ...
        # Conv stack: InstanceNorm1d normalises each sample, tanh bounds activations
        h = torch.tanh(self.norm1(self.conv1(x.unsqueeze(1))))
        h = torch.tanh(self.norm2(self.conv2(h)))
        h = torch.tanh(self.norm3(self.conv3(h)))
    ...
    def decode_poles_residues(self, z):
        ...
        h = torch.tanh(self.z_projector(z))
        h = torch.tanh(self.second_projector(h))
...
if __name__ == "__main__":
    ...
    model = VariationalAutoEncoder2(input_dim, num_poles, beta, N_nodes)
    ...
    print("G_recon shape :", G_recon.shape)
